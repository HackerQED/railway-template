...
import {
  ...
  addUpgradeCredits,
  expireSubscriptionBalance,
} from '@/credits/credits';
import { getCreditPackageById } from '@/credits/server';
import { CREDIT_TRANSACTION_TYPE } from '@/credits/types';
import { getDb } from '@/db';
import { payment, user, userConversion } from '@/db/schema';
...
export class StripeProvider implements PaymentProvider {
  ...
  private async onInvoicePaid(invoice: Stripe.Invoice): Promise<void> {
    ...
    try {
      ...
      // Add pending purchase report for Google Ads conversion tracking
      await this.addPendingPurchaseReport(paymentRecord.userId, invoice);
    } catch (error) {
  ...
  private async updateSubscriptionPayment(
    ...
    try {
      let subscriptionId = this.extractSubscriptionId(invoice);

      // If extractSubscriptionId returns null, try to use paymentRecord.subscriptionId
      ...
      // Skip credit processing for upgrade/downgrade invoices —
      // credits are handled in onUpdateSubscription via addUpgradeCredits
      if (invoice.billing_reason === 'subscription_update') {
        console.log(
          '<< Skipping credit processing for subscription_update invoice'
        );
      } else {
  ...
  private async processCreditPurchase(
    ...
    // Add credits to user's balance (purchased credits never expire)
  ...
  private async onUpdateSubscription(
    ...
    // Query existing payment record to detect plan change
    const db = await getDb();
    ...
    // Detect upgrade: priceId changed → supplement credit difference
    if (existingPayment && existingPayment.priceId !== priceId) {
      console.log(
        `Plan change detected: ${existingPayment.priceId} → ${priceId}`
      );
      await addUpgradeCredits({
        userId: existingPayment.userId,
        oldPriceId: existingPayment.priceId,
        newPriceId: priceId,
      });
    }
  ...
  private async onDeleteSubscription(
    ...
    if (result.length > 0) {
      ...
      // Expire subscription balance (set to 0)
      const userId = result[0].userId;
      await expireSubscriptionBalance({
        userId,
        description: `Subscription expired: ${stripeSubscription.id}`,
      });
    } else {
  ...
  private extractSubscriptionId(invoice: Stripe.Invoice): string | null {
    // Dahlia API: invoice.parent.subscription_details.subscription
    const subscriptionDetails = invoice.parent?.subscription_details;
    if (subscriptionDetails?.subscription) {
      const sub = subscriptionDetails.subscription;
      if (typeof sub === 'string') {
        console.log(
          `invoice.parent.subscription_details.subscription is string: ${sub}`
        );
        return sub;
      }
      if (typeof sub === 'object' && 'id' in sub) {
        console.log(
          `invoice.parent.subscription_details.subscription is object: ${sub.id}`
        );
        return sub.id;
      }
    }
    ...
    for (const lineItem of lineItems) {
      // lineItem.subscription still exists in dahlia
      ...
      const subItemDetails = lineItem.parent?.subscription_item_details;
      if (subItemDetails?.subscription) {
        console.log(
          `invoice.lineItem.parent.subscription_item_details.subscription: ${subItemDetails.subscription}`
        );
        return subItemDetails.subscription;
      }
    }
  ...
  /**
   * Add a pending purchase report to user_conversion for Google Ads conversion tracking.
   * The frontend ConversionTracker will consume and report these on any page load.
   */
  private async addPendingPurchaseReport(
    userId: string,
    invoice: Stripe.Invoice
  ): Promise<void> {
    try {
      ...
      // Upsert: create user_conversion if not exists, append report
      const existing = await db
        .select({
          id: userConversion.id,
          pendingPurchaseReports: userConversion.pendingPurchaseReports,
          purchaseAt: userConversion.purchaseAt,
          purchaseAmountCents: userConversion.purchaseAmountCents,
        })
        .from(userConversion)
      ...
      if (existing[0]) {
        const reports = Array.isArray(existing[0].pendingPurchaseReports)
          ? existing[0].pendingPurchaseReports
          : [];

        // Deduplicate by transactionId
        const alreadyExists = reports.some(
          (r: any) => r.transactionId === transactionId
        );
        if (alreadyExists) {
          console.log('Purchase report already exists for:', transactionId);
          return;
        }
        ...
        await db
          ...
          .set({
            pendingPurchaseReports: [...reports, report],
            purchaseAt: existing[0].purchaseAt ?? new Date(),
            purchaseAmountCents:
              (existing[0].purchaseAmountCents ?? 0) + amountCents,
            updatedAt: new Date(),
          })
          .where(eq(userConversion.id, existing[0].id));
      } else {
        await db.insert(userConversion).values({
          id: nanoid(),
          userId,
          pendingPurchaseReports: [report],
          purchaseAt: new Date(),
          purchaseAmountCents: amountCents,
        });
      }

      console.log('Added pending purchase report for user:', userId, report);
    } catch (error) {
      // Non-fatal: don't break the payment flow for tracking failures
      console.error('Failed to add pending purchase report:', error);
    }
  }
}
