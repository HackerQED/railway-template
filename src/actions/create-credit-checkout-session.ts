...
export const createCreditCheckoutSession = userActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    try {
      if (!websiteConfig.credits.enablePackagesForFreePlan) {
        const hasPremiumAccess = await checkPremiumAccess(currentUser.id);
        if (!hasPremiumAccess) {
          return {
            success: false,
            error: 'Subscription required to purchase credit packages',
          };
        }
      }
