...
export const createCheckoutAction = userActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    try {
      ...
      // Verify priceId is not empty (can happen when env vars are missing)
      if (!priceId) {
        console.error(
          `Checkout attempted with empty priceId for plan ${planId}`
        );
        return {
          success: false,
          error: 'Price configuration is incomplete',
        };
      }
      ...
      const successUrl = getUrlWithLocale(
        `${Routes.Payment}?session_id={CHECKOUT_SESSION_ID}&callback=${Routes.Pricing}`,
        locale
      );
